...
fn parse_line(input: &str) -> IResult<&str, (Valve, Vec<(char, char)>)> {
    let p = tuple((tag("Valve "), parse_valve_name, tag(" has flow rate="),
		  parse_decimal, alt((tag("; tunnels lead to valves "), tag("; tunnel leads to valve "))), parse_valve_name_list));
    map(p, |(_, name, _, rate, _, list)| (Valve { name, rate }, list))(input)
}
...
fn traverse_with_elephant(dist: &HashMap<(Valve, Valve), isize>, state: State2, max_gain: &mut isize) {
	...
	*max_gain = (*max_gain).max(state.gain + extra_me + extra_elephant);	
	...
	for new_me in &state.closed {
		...
		let new_remaining_steps_elephant = state.remaining_steps - dist[&(state.elephant, *new_elephant)] - 1;
		let new_remaining_steps = new_remaining_steps_me.max(new_remaining_steps_elephant);
		let newstate = State2 { me: *new_me,
					elephant: *new_elephant,
					closed: new_closed,
					gain: state.gain + state.remaining_steps * (state.me.rate + state.elephant.rate),
					remaining_steps: new_remaining_steps,
	...
	for new_elephant in &state.closed {
	    ...
	    let new_remaining_steps_elephant = state.remaining_steps - dist[&(state.elephant, *new_elephant)] - 1;
	    let new_remaining_steps = new_remaining_steps_me.max(new_remaining_steps_elephant);
	    let newstate = State2 { me: state.me,
				    elephant: *new_elephant,
				    closed: new_closed,
...
fn all_shortest_paths(graph: &Map, start: Valve, goal: &HashSet<Valve>) -> HashMap<Valve, isize> {
	...
	if goal.contains(&valve) { dd.insert(valve, d); }

        // Important as we may have already found a better way
        if d > dist[&valve] { continue; }

        // For each node we can reach, see if we can find a way with
        // a lower cost going through this node
        for node in &graph[&valve] {
            let next = Dist { valve: *node, d: d + 1};
