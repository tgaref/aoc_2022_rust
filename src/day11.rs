...
use Operation::{ Mul, Add, MulOld, AddOld };

impl Operation {
    fn parse(input: &str) -> IResult<&str, Operation> {
	let tp = tuple((tag("  Operation: new = old "), alt((tag("+ "), tag("* "))), alt((tag("old"), digit1))));
...
fn parse_target(input: &str) -> IResult<&str, usize> {
    let p = tuple((alt((tag("    If true: throw to monkey "), tag("    If false: throw to monkey "))), digit1));
...
fn simulate(mut monkeys: Vec<Monkey>, steps: usize, modulus: Option<u64>, reduce: Option<u64>) -> String {
...
fn round(mut monkeys: Vec<Monkey>, modulus: Option<u64>, reduce: Option<u64>) -> (Vec<Monkey>, Vec<u64>) {
	...
	for worry_level in &monkey.items {
	    let new_worry_level = update_worry_level(*worry_level, monkey.operation, modulus, reduce);
