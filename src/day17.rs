...
fn move_down(state: State) -> (State, bool) {
    ...
    let new_block = state.block.0.iter().map(|(x,y)| (x-1, *y)).collect::<Vec<_>>();

    (State { block: Block(new_block), grid: state.grid, highest: state.highest }, true)
}
...
fn move_sideways(state: State, dir: char) -> State {
	...
	'>' => {
	    ...
	    let new_block = state.block.0.iter().map(|(x,y)| (*x, y+1)).collect::<Vec<_>>();
	    State { block: Block(new_block), ..state }
	},
	'<' => {
	    for (x,y) in &state.block.0 {
...
fn initialize_block(block: &Block, highest: usize) -> Block {
    Block(block.0.iter().map(|(x,y)| (x + highest + 4, y + 2)).collect())
}

fn simulate(input: &str, blocks: Vec<Block>, nrocks: usize) -> usize {
    let mut block = initialize_block(&blocks[0], 0);
    let mut state = State { block, grid: HashSet::new(), highest: 0 };
    let mut moved: bool;
    let mut blk_idx = 1;
	...
	if !moved {
	    ...
	    blk_idx += 1;
	    state = State { block, grid, highest };
...
fn detect_cycle(input: &[char], blocks: Vec<Block>, nrocks: usize) -> usize {
    ...
    let repeat = Repeat { tops: vec![0,0,0,0,0,0,0], input_idx: 0 };
	...
	if !moved {
	    ...
	    for i in 0..7 {
		let block_max_x = state.block.0.iter().filter(|(_,y)| *y == i).max_by_key(|(x,_)| *x).unwrap_or(&(0,0)).0;
	    ...
	    let repeat = Repeat { tops: new_tops.clone(), input_idx };
...
impl Puzzle for Day17 {
	...
	let blocks: Vec<Block> = vec![Block::new(vec![(0,0), (0,1), (0,2), (0,3)]),
...
//	print_grid(&h.grid);
	...
	let h = detect_cycle(&self.input.chars().collect::<Vec<_>>(), blocks, 1000000000000);	
	format!("{:?}", h)
    }        
}
