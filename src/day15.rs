...
fn parse_int(input: &str) -> IResult<&str, i32> {
    map(tuple((many0_count(char('-')), digit1)),
	|(sign, s)| { let n = i32::from_str_radix(s,10).unwrap();
		      if sign % 2 == 0 {
			  n
		      } else {
			  -n
...
fn parse_position(input: &str) -> IResult<&str, Position> {
    map(tuple((tag("x="), parse_int, tag(", y="), parse_int)),
	|(_,x,_,y)| Position { x, y }
	)(input)
}

fn parse_line(input: &str) -> IResult<&str, (Position, Position)> {
    map(tuple((tag("Sensor at "), parse_position, tag(": closest beacon is at "), parse_position)),
	|(_, p1, _, p2)| (p1, p2)
    )(input)
}
