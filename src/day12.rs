...
impl Day12 {
    pub fn new() -> Day12 {
	Day12 { input: Grid::new(), start: (0,0), goal: (0,0) } 	
    }
...
fn neighbours_up(grid: &Map, pos: Position) -> Vec<Position> {
	...
	if p.0 >= 0 && p.0 < m && p.1 >= 0 && p.1 < n && grid[p.0 as usize][p.1 as usize] as u8 <= c + 1 {
	    neigh.push(p);
	}
    }
    neigh
...
fn dijkstra(grid: &Map, start: Position, goal: Position) -> Option<u64> {
    ...
    while let Some(State { cost, pos }) = heap.pop() {
        ...
        for node in neighbours_up(grid, pos) {
            let next = State { cost: cost + 1, pos: node };
...
fn all_shortest_paths(grid: &Map, start: Position) -> HashMap<Position, u64> {
    ...
    while let Some(State { cost, pos }) = heap.pop() {
        ...
        if grid[pos.0 as usize][pos.1 as usize] == 'a' { dd.insert(pos, cost); }
