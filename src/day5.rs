...
fn parse_instruction(input: &str) -> IResult<&str, Instruction> {
    map(
	tuple((tag("move "), parse_decimal, tag(" from "), parse_decimal, tag(" to "), parse_decimal)),
...
impl Puzzle for Day5 {
	...
	let mut state = self.state.clone();
	    ...
	    let l = from.len();
	    let chunk = from[l-instr.amount..].to_string().chars().rev().collect::<String>();
