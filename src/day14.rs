...
impl Add<(isize, isize)> for Position {
	...
	Position { x: self.x + other.0, y: self.y + other.1 }
    }
}
...
fn parse_position(input: &str) -> IResult<&str, Position> {
    map(separated_pair(digit1, char(','), digit1),
	|(x,y)| Position { x: isize::from_str_radix(x,10).unwrap(),
...
fn get_positions(Position { x: x1, y: y1 }: Position, Position { x: x2, y: y2 }: Position) -> Vec<Position> {
...
enum Direction { RD, LD, D }

use Direction::{ RD, LD, D };
