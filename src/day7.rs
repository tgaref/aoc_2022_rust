...
impl Contents {
    fn new() -> Contents {
	Contents { dirs: Vec::new(), files: Vec::new() }
    }
}
...
fn parse_file(line: &str) -> File {
    let size = line.chars().take_while(|c| c.is_digit(10)).collect::<String>().parse::<usize>().unwrap();
    let name = line.chars().skip_while(|c| c.is_digit(10) || c.is_whitespace()).collect::<String>();
...
impl Puzzle for Day7 {
	...
	format!("{:}", dir_sizes.values().filter(|&v| *v <= 100000).sum::<usize>())
    }      				    
...
fn traverse(root: &str, data: &HashMap<String, Contents>, dir_sizes: &mut HashMap<String, usize>) -> usize {
